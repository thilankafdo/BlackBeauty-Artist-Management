...
const Dashboard: React.FC<DashboardProps> = ({ onNavigate, role, gigs, heroImage, heroScale }) => {
  ...
  React.useEffect(() => {
    fetch('http://localhost:3002/api/gmail/preview').then(r => r.json()).then(setEmails);
    fetch('http://localhost:3002/api/drive/files').then(r => r.json()).then(setDriveFiles);
  }, []);

  const handleSyncFinancials = async () => {
    setIsSyncing(true);
    try {
      const res = await fetch('http://localhost:3002/api/sync/sheets', { method: 'POST' });
      if (res.ok) alert('Financials synced to Google Sheets!');
      else alert('Google Sheets sync failed. Please check your credentials in settings.');
    } catch (e) {
      alert('Sync failed.');
    } finally {
      setIsSyncing(false);
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 relative p-10 md:p-16 rounded-[4rem] bg-zinc-900/20 border border-zinc-800/50 overflow-hidden group">
        <div className="absolute right-0 top-0 h-full w-full pointer-events-none overflow-hidden">
          ...
          <img
            ...
            className="h-full w-full object-cover object-center opacity-30 grayscale-[0.5] group-hover:scale-[1.05] transition-transform duration-1000"
            alt="Artist Hero"
          />
        </div>

        <div className="space-y-4 z-20 flex flex-col items-start relative overflow-visible px-4">
          <div className="relative">
            <span className="font-logo text-6xl md:text-9xl neon-logo-glow leading-[1] inline-block whitespace-nowrap">
              Black Beauty
            </span>
          </div>
        ...
        <div className="flex flex-wrap gap-4 z-20">
          ...
          <button
            ...
            disabled={isSyncing}
            className={`group px-8 py-5 bg-zinc-950 border border-zinc-800 rounded-3xl text-[10px] font-black uppercase tracking-widest text-zinc-400 hover:text-white shadow-xl hover:bg-zinc-900 transition-all flex items-center gap-3 active:scale-95 disabled:opacity-50`}
          >
            <RefreshCw className={`w-4 h-4 text-amber-500 ${isSyncing ? 'animate-spin' : 'group-hover:rotate-180'} transition-transform`} />
            Sync Financials
          </button>
          <button
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'Calendar', icon: Calendar, view: 'gigs', color: 'bg-yellow-500/10 text-yellow-400 border-yellow-500/20 shadow-yellow-500/5' },
          { label: 'Quotations', icon: Receipt, view: 'quotation', color: 'bg-zinc-800/80 text-zinc-400 border-zinc-700/50 shadow-zinc-500/5' },
          { label: 'Journal', icon: BookOpen, view: 'journal', color: 'bg-amber-500/10 text-amber-400 border-amber-500/20 shadow-amber-500/5' },
          { label: 'EPK Hub', icon: FileText, view: 'epk', color: 'bg-zinc-800/80 text-zinc-400 border-zinc-700/50 shadow-zinc-500/5' },
        ].map((action, i) => (
          <button
            key={i}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-8 space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              { label: 'Streams', value: '1.2M', growth: '+12%', icon: PlayCircle, color: 'text-yellow-500' },
              { label: 'Total Gigs', value: gigs.length.toString(), growth: '+2', icon: Calendar, color: 'text-zinc-500' },
              { label: 'Revenue YTD', value: 'LKR 4.2M', growth: '+8%', icon: TrendingUp, color: 'text-amber-600' },
            ].map((stat, i) => (
              <div key={i} className="bg-zinc-900/40 border border-zinc-800/50 p-7 rounded-[2.5rem] hover:border-yellow-500/30 transition-all shadow-xl group">
        ...
        <div className="lg:col-span-4 space-y-8">
          ...
          <div className="bg-zinc-900/30 border border-zinc-800 p-8 rounded-[3rem] shadow-2xl space-y-8">
            <div className="flex items-center justify-between">
              <h3 className="font-black text-xl text-white font-syncopate tracking-tighter uppercase">Communication</h3>
              <div className="w-8 h-8 rounded-full bg-zinc-800 flex items-center justify-center border border-zinc-700 animate-pulse">
                <div className="w-2 h-2 bg-purple-500 rounded-full"></div>
              </div>
            </div>
            <div className="space-y-4">
              {emails.length > 0 ? emails.map((email) => (
                <div key={email.id} className="relative p-5 bg-black/40 border border-zinc-900 rounded-[1.75rem] group hover:border-purple-500/30 transition-all cursor-pointer">
                  {email.unread && <div className="absolute top-5 right-5 w-2 h-2 bg-purple-500 rounded-full shadow-[0_0_12px_#a855f7]"></div>}
                  <div className="flex items-start gap-4">
                    <div className="p-3 rounded-xl border border-zinc-800 bg-zinc-900 text-purple-400 group-hover:scale-110 transition-transform">
                      <Mail className="w-4 h-4" />
                    </div>
                    <div className="flex-1 min-w-0 text-left">
                      <div className="flex justify-between items-center mb-1">
                        <p className="text-[10px] font-black text-zinc-400 uppercase truncate">{email.from}</p>
                        <span className="text-[8px] font-bold text-zinc-600 uppercase">{email.date}</span>
                      </div>
                      <p className="text-xs font-bold text-white leading-tight truncate">{email.subject}</p>
                      <p className="text-[9px] text-zinc-600 truncate mt-1 leading-relaxed">{email.snippet}</p>
                    </div>
                  </div>
                </div>
              )) : (
                <div className="py-12 flex flex-col items-center justify-center border-2 border-dashed border-zinc-800/50 rounded-[2rem]">
                  <Mail className="w-8 h-8 text-zinc-800 mb-3" />
                  <p className="text-[10px] text-zinc-600 font-extrabold uppercase tracking-widest">Inbox Synchronized</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-zinc-900/30 border border-zinc-800 p-8 rounded-[3rem] shadow-2xl space-y-8">
            <div className="flex items-center justify-between">
              <h3 className="font-black text-xl text-white font-syncopate tracking-tighter uppercase leading-none">Asset Vault</h3>
              <FolderSync className="w-4 h-4 text-zinc-600" />
            </div>
            <div className="space-y-4">
              {driveFiles.length > 0 ? driveFiles.map((file) => (
                <a
                  ...
                  className="flex items-center gap-4 p-5 bg-black/40 border border-zinc-900 rounded-[1.75rem] group hover:border-purple-500/30 transition-all active:scale-95"
                >
                  <div className="p-3 rounded-xl border border-zinc-800 bg-zinc-900 group-hover:bg-zinc-800/80 transition-colors">
                    <FileText className="w-4 h-4 text-zinc-400" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-bold text-white truncate uppercase tracking-tight">{file.name}</p>
                    <p className="text-[8px] text-zinc-600 font-black uppercase mt-1 tracking-widest">
                      Modified {new Date(file.modifiedTime).toLocaleDateString()}
                    </p>
                  </div>
                  <ExternalLink className="w-3 h-3 text-zinc-700 group-hover:text-purple-500 transition-colors" />
                </a>
              )) : (
                <div className="py-12 flex flex-col items-center justify-center border-2 border-dashed border-zinc-800/50 rounded-[2rem]">
                  <FolderSync className="w-8 h-8 text-zinc-800 mb-3" />
                  <p className="text-[10px] text-zinc-600 font-extrabold uppercase tracking-widest">No Cloud Assets Found</p>
                </div>
              )}
            </div>
          </div>
